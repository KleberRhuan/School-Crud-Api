...
public class CsvImportWebSocketController {
  ...
  public void subscribe(@DestinationVariable String jobId,
                        SimpMessageHeaderAccessor header) {

    getUserId(header).ifPresentOrElse(userId -> {
      ...
      sendNotification(userId, buildNotification(
              job, "Inscrito com sucesso no job " + jobId
      ));
  ...
  public void unsubscribe(@DestinationVariable String jobId,
                          SimpMessageHeaderAccessor header) {

    getUserId(header).ifPresentOrElse(userId -> {
      UUID uuid = parseJobId(jobId, userId);
      if (uuid != null) {
        subscriptions.getOrDefault(userId, Collections.emptySet()).remove(uuid);
      }

      sendNotification(userId, new CsvImportNotification(
              null, userId, null, null, 0, 0, 0,
              "Subscrição cancelada para job " + jobId, MSG_INFO, Instant.now()
      ));
  ...
  public void status(@Payload Map<String, String> payload,
                     SimpMessageHeaderAccessor header) {
  ...
  public void listJobs(SimpMessageHeaderAccessor header) {
    getUserId(header).ifPresentOrElse(userId -> {
      sendNotification(userId, new CsvImportNotification(
              null, userId, null, null, 0, 0, 0,
              "Para listar jobs, use GET /api/v1/csv/jobs", MSG_INFO, Instant.now()
      ));
  ...
  private Optional<Long> getUserId(SimpMessageHeaderAccessor header) {
    Authentication auth = (Authentication)
            header.getSessionAttributes().get("user");
  ...
  private void sendNotification(Long userId, CsvImportNotification notif) {
    messagingTemplate.convertAndSendToUser(
            userId.toString(), DEST_PREFIX, notif
    );
  }

  private void sendError(Long userId, String msg) {
    sendNotification(userId, new CsvImportNotification(
            null, userId, null, null, 0, 0, 0, msg, MSG_ERROR, Instant.now()
    ));
  }

  private CsvImportNotification buildNotification(
          CsvImportJob job, String message
  ) {
