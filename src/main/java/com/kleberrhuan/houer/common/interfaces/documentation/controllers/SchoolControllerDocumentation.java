...
@Tag(
  name = "Gerenciamento de Escolas",
  description = "Endpoints para operações CRUD com dados de escolas, incluindo consultas avançadas e métricas"
)
public interface SchoolControllerDocumentation {
  ...
  @ApiResponses(
    {
      @ApiResponse(
        ...
        content = @Content(
          schema = @Schema(implementation = SchoolDtoSchema.class)
        )
      ),
      ...
      @ApiResponse(
        responseCode = "409",
        description = "Conflito - Código da escola já existe",
        content = @Content(
  ...
  ResponseEntity<SchoolDto> createSchool(
    @RequestBody(
      ...
      required = true,
      content = @Content(
        schema = @Schema(implementation = SchoolCreateRequestSchema.class),
        examples = @io.swagger.v3.oas.annotations.media.ExampleObject(
          name = "Escola Exemplo",
          summary = "Exemplo de criação de escola",
          value = """
          {
            "code": 12345678,
  ...
  @Operation(
    summary = "Atualizar escola existente",
    description = """
  ...
  ResponseEntity<SchoolDto> updateSchool(
    @Parameter(
      description = "Código único da escola a ser atualizada",
      required = true,
      example = "12345678"
    ) @PathVariable Long code,
    @RequestBody(
      description = "Dados para atualização da escola (campos opcionais)",
      required = true,
      content = @Content(
        schema = @Schema(implementation = SchoolUpdateRequestSchema.class),
        examples = @io.swagger.v3.oas.annotations.media.ExampleObject(
          name = "Atualização Exemplo",
  ...
  @Operation(
    summary = "Buscar escola por código",
    description = """
  ...
  @Operation(
    summary = "Listar escolas com filtros",
    description = """
  ...
  ResponseEntity<PaginatedResponse<List<SchoolDto>>> getAllSchools(
    ...
    @Parameter(
      description = "Nome da escola (busca parcial)",
      example = "MUNICIPAL"
    ...
    @Parameter(description = "Sigla do estado", example = "SP") @RequestParam(
      required = false
    ) String stateAbbreviation,
    @Parameter(
      description = "Status operacional (1=Ativa, 2=Paralisada)",
      example = "1"
    ...
    @Parameter(
      description = "Tipo da escola (1=Infantil, 2=Fundamental)",
      example = "2"
    ...
    @Parameter(
      description = "Dependência administrativa específica",
      example = "Municipal"
    ) @RequestParam(required = false) String administrativeDependence,
    @Parameter(
      description = "Localização/município",
      example = "Centro"
    ) @RequestParam(required = false) String location,
    @Parameter(description = "Situação da escola", example = "1") @RequestParam(
      required = false
    ) String situation
  );

  @Operation(
    summary = "Obter métricas da escola",
    description = """
  ...
  @ApiResponses(
    {
      @ApiResponse(
        responseCode = "200",
        description = "Métricas da escola retornadas com sucesso",
        content = @Content(
          schema = @Schema(implementation = SchoolMetricsDtoSchema.class)
        )
      ),
      @ApiResponse(ref = "#/components/responses/BadRequest"),
      @ApiResponse(ref = "#/components/responses/NotFound"),
  ...
  ResponseEntity<Void> deleteSchool(
    @Parameter(
      description = "Código único da escola a ser excluída",
      required = true,
      example = "12345678"
    ) @PathVariable Long code
  );
